<script lang="ts">
	...
	const prayersEndpoints = [
		...
		{
			method: 'GET',
			path: '/api/v1/prayers?category={categoria}',
			desc: 'Filtrar orações por categoria'
		},
	...
	const liturgyEndpoints = [
		...
		{
			method: 'GET',
			path: '/v2/?dia={dia}&mes={mes}&ano={ano}',
			desc: 'Liturgia de data específica'
		},
	...
	const bibleEndpoints = [
		...
		{ method: 'GET', path: '/api/v1/bible/books/{book}', desc: 'Detalhes de um livro' },
		{
			...
			desc: 'Busca textual (mín. 3 caracteres)'
		},
...
<main>
			...
			<aside class="sidebar w-56 shrink-0">
				<nav class="sticky top-28 flex flex-col gap-1 py-2">
					{#each sidebarSections as section}
						{#if section.children}
							<span
								class="text-xs font-semibold text-primary uppercase tracking-wider pt-4 pb-1 px-3"
								>{section.label}</span
							>
			...
			<div class="flex-1 min-w-0">
				...
				<section id="backend" class="scroll-mt-24 pb-16">
					<div class="flex flex-col mb-8">
						...
						>
						<h2
							class="text-3xl md:text-4xl font-serif font-medium leading-tight text-text-light max-w-3xl"
						>
							Backend — Paróquias
						</h2>
						<p class="mt-4 text-base text-text-secondary max-w-2xl leading-relaxed">
							Base URL: <code
								class="text-primary bg-primary-dim px-2 py-0.5 rounded text-sm font-mono"
								>https://api.ordinarium.com.br</code
							>
						</p>
						<p class="mt-2 text-sm text-text-secondary max-w-2xl leading-relaxed">
							API REST para consulta de paróquias da Arquidiocese de Teresina. Desenvolvida com
							NestJS e Drizzle ORM.
						</p>
					</div>

					<!-- Endpoints Table -->
					<div
						class="bg-bg-dark-card border border-border-gold/20 rounded-xl overflow-hidden mb-10"
					>
						<div class="p-5 border-b border-border-gold/20 flex items-center gap-2">
							<List class="w-4 h-4 text-primary" />
							<h3 class="text-sm font-semibold text-text-light uppercase tracking-wider">
								Endpoints
							</h3>
						</div>
							...
							<table class="w-full text-sm">
								<thead>
									<tr class="border-b border-border-gold/10">
										<th
											class="text-left p-4 text-text-secondary font-medium uppercase tracking-wider text-xs"
											>Método</th
										>
										<th
											class="text-left p-4 text-text-secondary font-medium uppercase tracking-wider text-xs"
								...
								<tbody>
									{#each backendEndpoints as ep}
										<tr
											class="border-b border-border-gold/10 last:border-0 hover:bg-white/[0.02] transition-fast"
										>
											<td class="p-4">
												<span
													class="inline-block px-2 py-0.5 rounded text-xs font-mono font-bold bg-green-900/30 text-green-400"
													>GET</span
												>
											</td>
  ...
  "longitude": "-42.8019"
					...
					<div
						id="ep-parish-model"
						class="scroll-mt-24 mt-12 bg-bg-dark-card border border-border-gold/20 rounded-xl p-6 md:p-8"
					>
						<h3 class="font-serif text-xl font-medium text-text-light mb-6">
							Modelo de Dados — Parish
						</h3>
						<div class="overflow-x-auto">
							<table class="w-full text-sm">
								<thead>
									<tr class="border-b border-border-gold/10">
										<th
											class="text-left p-2 text-text-secondary font-medium text-xs uppercase tracking-wider"
											>Campo</th
										>
										<th
											class="text-left p-2 text-text-secondary font-medium text-xs uppercase tracking-wider"
								...
								<tbody>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">id</td><td
											class="p-2 text-xs text-text-secondary">integer</td
										><td class="p-2 text-xs text-text-secondary">ID único auto-incremental</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">forania</td><td
											class="p-2 text-xs text-text-secondary">string</td
										><td class="p-2 text-xs text-text-secondary">Forania à qual pertence</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">tipo</td><td
											class="p-2 text-xs text-text-secondary">string</td
										><td class="p-2 text-xs text-text-secondary"
											>paroquia, capela, capelania, diaconia, area_pastoral, santuario</td
										></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">nome</td><td
											class="p-2 text-xs text-text-secondary">string</td
										><td class="p-2 text-xs text-text-secondary">Nome oficial da paróquia</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">endereco</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Endereço completo</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">bairro</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Bairro</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">cep</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">CEP</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">telefone</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Telefone(s) de contato</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">email</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">E-mail de contato</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">instagram</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Perfil do Instagram</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">site</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Site oficial</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">paroco</td><td
											class="p-2 text-xs text-text-secondary">string | null</td
										><td class="p-2 text-xs text-text-secondary">Pároco atual</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">vigarios</td><td
											class="p-2 text-xs text-text-secondary">string[]</td
										><td class="p-2 text-xs text-text-secondary">Vigários</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">diaconos</td><td
											class="p-2 text-xs text-text-secondary">string[]</td
										><td class="p-2 text-xs text-text-secondary">Diáconos</td></tr
									>
									<tr class="border-b border-border-gold/10"
										><td class="p-2 font-mono text-xs text-text-light">latitude</td><td
											class="p-2 text-xs text-text-secondary">number | null</td
										><td class="p-2 text-xs text-text-secondary">Latitude geográfica</td></tr
									>
									<tr
										><td class="p-2 font-mono text-xs text-text-light">longitude</td><td
											class="p-2 text-xs text-text-secondary">number | null</td
										><td class="p-2 text-xs text-text-secondary">Longitude geográfica</td></tr
									>
								</tbody>
							</table>
						</div>
					...
					>
						{#snippet children()}
							<div class="mt-3">
								...
								<pre
									class="bg-gray-950 border border-border-gold/10 rounded-lg p-3 overflow-x-auto"><code
										class="text-xs font-mono text-green-400"
										>curl "https://api.ordinarium.com.br/api/v1/prayers?category=Orações Marianas"</code
									></pre>
							</div>
						{/snippet}
					</EndpointCard>
  ...
  "questions": [
    ...
    "Tenho sido paciente, amoroso e compreensivo com meu cônjuge..."
								...
								<table class="w-full text-sm">
									...
									<tbody>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">id</td><td
												class="p-2 text-xs text-text-secondary">integer</td
											><td class="p-2 text-xs text-text-secondary">ID único da oração</td></tr
										>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">slug</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Identificador URL-friendly</td
											></tr
										>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">title</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Título da oração</td></tr
										>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">category</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Categoria temática</td></tr
										>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">text</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Texto completo da oração</td></tr
										>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">source</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Origem/fonte da oração</td></tr
										>
										<tr
											><td class="p-2 font-mono text-xs text-text-light">language</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Idioma (padrão: pt-BR)</td></tr
										>
									</tbody>
								</table>
							</div>
								...
								<table class="w-full text-sm">
									...
									<tbody>
										<tr class="border-b border-border-gold/10"
											><td class="p-2 font-mono text-xs text-text-light">category</td><td
												class="p-2 text-xs text-text-secondary">string</td
											><td class="p-2 text-xs text-text-secondary">Nome da categoria</td></tr
										>
										<tr
											><td class="p-2 font-mono text-xs text-text-light">questions</td><td
												class="p-2 text-xs text-text-secondary">array</td
											><td class="p-2 text-xs text-text-secondary"
												>Perguntas reflexivas (<code class="text-primary">{QUESTION_SCHEMA}</code
												>)</td
											></tr
										>
									</tbody>
					...
					<div
						id="ep-errors"
						class="scroll-mt-24 mt-6 bg-bg-dark-card border border-border-gold/20 rounded-xl p-6 md:p-8"
					>
						...
						<div class="overflow-x-auto">
							<table class="w-full text-sm">
								<thead>
									<tr class="border-b border-border-gold/10">
										<th
											class="text-left p-3 text-text-secondary font-medium uppercase tracking-wider text-xs"
											>Status</th
										>
										<th
											class="text-left p-3 text-text-secondary font-medium uppercase tracking-wider text-xs"
								...
								<tbody>
									<tr class="border-b border-border-gold/10">
										...
										<td class="p-3 text-xs text-text-secondary"
											>Oração ou categoria não encontrada</td
										>
									</tr>
									<tr>
										<td class="p-3"><span class="font-mono text-xs text-red-400">422</span></td>
										<td class="p-3 text-xs text-text-secondary"
											>Parâmetros inválidos (ex: query string vazia)</td
										>
									</tr>
								</tbody>
							</table>
				...
				<section id="bible" class="scroll-mt-24 pb-16">
					<div class="flex flex-col mb-8">
						...
						<p class="mt-2 text-sm text-text-secondary max-w-2xl leading-relaxed">
							Bíblia Sagrada Católica completa — 73 livros (incluindo deuterocanônicos) com busca
							textual, capítulos e versículos. Suporta busca por nome ou abreviação do livro.
						</p>
					</div>
  ...
  "text": "Porque Deus amou o mundo de tal modo, que deu o seu Filho Unigênito, para que todo aquele que nele crê não pereça, mas tenha a vida eterna."
				...
				<section id="santo" class="scroll-mt-24 pb-16">
					<div class="flex flex-col mb-8">
						...
						<p class="mt-2 text-sm text-text-secondary max-w-2xl leading-relaxed">
							API de web scraping que obtém informações sobre o santo do dia a partir do site A12.
							Retorna nome, imagem, história, reflexão e oração.
						</p>
					</div>
      ...
      "oracao": "Ó Deus, que pelo ministério..."
				...
				<section id="liturgia" class="scroll-mt-24 pb-8">
					<div class="flex flex-col mb-8">
						...
						<p class="mt-4 text-base text-text-secondary max-w-2xl leading-relaxed">
							API pública mantida por <a
								href="https://github.com/Dancrf"
								target="_blank"
								rel="noopener noreferrer"
								class="text-primary hover:underline">Dancrf</a
					...
					<div class="bg-bg-dark-card border border-border-gold/20 rounded-xl p-6 md:p-8 mb-8">
						<p class="text-sm text-text-secondary leading-relaxed mb-4">
							Esta API fornece as orações e leituras do dia da Santa Missa (liturgia diária).
							Suporta consulta por data específica, período de dias, e versões alternativas de
							leituras.
						</p>
						<div class="flex flex-wrap gap-3">
							<a
								href="https://liturgia.up.railway.app/v2/"
								target="_blank"
								rel="noopener noreferrer"
								class="inline-flex items-center gap-1.5 text-xs text-primary border border-border-gold rounded-lg px-3 py-1.5 hover:bg-primary/10 transition-fast"
							>
								liturgia.up.railway.app/v2 <ExternalLink class="w-3 h-3" />
							</a>
							<a
								href="https://github.com/Dancrf/liturgia-diaria/blob/main/docs/v2/README.md"
								target="_blank"
								rel="noopener noreferrer"
								class="inline-flex items-center gap-1.5 text-xs text-text-secondary border border-border-gold/30 rounded-lg px-3 py-1.5 hover:text-primary hover:border-primary transition-fast"
							>
								Documentação v2 <ExternalLink class="w-3 h-3" />
							</a>
							<a
								href="https://github.com/Dancrf/liturgia-diaria/blob/main/docs/v3/README.md"
								target="_blank"
								rel="noopener noreferrer"
								class="inline-flex items-center gap-1.5 text-xs text-text-secondary border border-border-gold/30 rounded-lg px-3 py-1.5 hover:text-primary hover:border-primary transition-fast"
							>
					...
					<div class="bg-bg-dark-card border border-border-gold/20 rounded-xl p-6 md:p-8">
						...
						<pre
							class="bg-gray-950 border border-border-gold/10 rounded-lg p-4 overflow-x-auto"><code
...
{'}'}</code
						...
						<p class="text-xs text-text-secondary mt-4">
							Consulte a <a
								...
								target="_blank"
								rel="noopener noreferrer"
								class="text-primary hover:underline">documentação oficial</a
							> para detalhes completos de todos os campos.
						</p>
					</div>
				</section>
			</div>
