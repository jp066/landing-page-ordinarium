...
<footer id="contato" class="w-full bg-[#0B0B0C] text-text-light pt-20 pb-8 px-6 md:px-12 border-t border-border-gold/20">
	<div class="max-w-7xl mx-auto">
		
		<!-- Footer Grid -->
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 pb-16 border-b border-white/5">
			
			<!-- Column 1: Logo & Description (spans 4 columns on large screens) -->
			<div class="lg:col-span-4 flex flex-col items-start">
				...
				<div class="flex items-center gap-4">
					<a
						...
						class="w-8 h-8 rounded-full bg-white/5 hover:bg-primary/20 hover:text-primary flex items-center justify-center text-text-secondary transition-fast"
						aria-label="GitHub"
					>
			...
			<!-- Columns 2, 3, 4: Dynamic Links (spans 8 columns on large screens) -->
			<div class="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-8">
				{#each footerColumns as col}
					<div class="flex flex-col">
						<span class="text-xs font-semibold text-text-light uppercase tracking-wider mb-4">
							{col.title}
						</span>
						<nav class="flex flex-col gap-3">
							{#each col.links as link}
								<a
									href={link.href}
									{...(link.target ? { target: link.target, rel: link.rel || 'noopener noreferrer' } : {})}
									class="text-xs md:text-sm text-text-secondary hover:text-primary transition-fast"
								>
									{link.label}
								</a>
							{/each}
		...
		<div class="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
			<div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-6">
				...
				<div class="flex items-center gap-4">
					<a href="/privacidade" class="text-xs text-text-secondary hover:text-primary transition-fast">
						Privacidade
					</a>
			...
			<span class="text-xs text-text-secondary flex items-center gap-1">
				Desenvolvido por <a href="https://www.linkedin.com/in/joao-pedro-matos-dev/" target="_blank" rel="noopener noreferrer" class="text-primary hover:underline transition-fast">João Pedro Matos</a>
			</span>
		</div>
